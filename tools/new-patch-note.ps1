...
# Run from the Empire-UrbanRural/ directory (the one containing About/, 1.6/, etc.)
...
if (-not (Test-Path $defsDir) -or -not (Test-Path $aboutPath)) {
    Write-Host "ERROR: Run this script from the Empire-UrbanRural/ directory (the one containing About/ and 1.6/)." -ForegroundColor Red
    exit 1
}
...
$defName = "UR_${major}_${minor}_${patch}"
...
${indent}<FactionColonies.PatchNoteDef ParentName="URPatchBase">
...
$csprojPath = "1.6\Source\Core\Empire.UrbanRural.csproj"
if (Test-Path $csprojPath) {
    ...
    Write-Host "  Empire.UrbanRural.csproj AssemblyVersion/FileVersion updated to $assemblyVersion" -ForegroundColor Cyan
} else {
